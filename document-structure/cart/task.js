...
for(let i = 0; i <product.length; i++){
	product[i].querySelector("div.product__add").onclick = function(){
		...
		if(item >= 0){
			cartProduct[item].querySelector("img").src = product[i].querySelector("img").src;
		} else{
			cartProducts.insertAdjacentHTML(beforeEnd, `<div class="cart__product" data-id="${product[i].dataset.id}"> <img class="cart__product-image" src="${product[i].querySelector("img").src}"> <div class="cart__product-count">${product[i].querySelector("div.product__quantity-value").innerText}</div> </div>`)
		}
	}
};
